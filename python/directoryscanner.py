...
def multinomial_filter (in_path, out_path, node):
    ...
    try:
        ...
        subprocess.check_call (['/usr/bin/bpsh', str(node),'/opt/julia-040f3ab6b7/bin/julia/mcmc.jl', '-t', '0.005', '-p', '0.999', '-f', filtered_out, '-j', json_out, in_path], stdout = subprocess.DEVNULL, stderr = subprocess.DEVNULL) 
    except subprocess.CalledProcessError as e:
